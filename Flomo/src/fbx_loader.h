...
//
//// =============================================================================
//// FBX Loader - Load FBX animation files directly into BVHData structures
//// =============================================================================
////
//// This loader reads FBX files using ufbx and populates the same BVHData
//// structure used by the BVH parser, allowing Flomo to load FBX files
//// seamlessly alongside BVH files.
////
//// Key implementation notes (lessons learned from fbx_converter):
////
//// 1. STATIC REFERENCE NODES: Many FBX files have a static "Reference" node
////    at the origin with no animation. We detect and skip these.
////
//// 2. UNIT SCALING: ufbx converts node_to_world to meters, but node_to_parent
////    stays in original units (typically cm). Scale offsets accordingly.
////
//// 3. EULER ANGLES: BVH uses ZXY rotation order. Decomposition formulas:
////    X = asin(M[2][1]), Y = atan2(-M[2][0], M[2][2]), Z = atan2(-M[0][1], M[1][1])
////
//// =============================================================================
//
...
//#include "raymath.h"
//
//// We need ufbx - assume it's available (added to project)
//#include "ufbx.h"
//
//#include <vector>
//#include <string>
//#include <cmath>
//
//// Define FBX_PROFILING to enable timing logs
...
//    } while(0)
//#define FBX_PROFILE_SCOPE(section_name) \
//    struct ProfileScope_##__LINE__ { \
//        double start; \
//        const char* name; \
//        ProfileScope_##__LINE__(const char* n) : start(GetTime()), name(n) {} \
//        ~ProfileScope_##__LINE__() { \
//            double elapsed = GetTime() - start; \
//            printf("[FBX Profiling] %s: %.3f ms\n", name, elapsed * 1000.0); \
//        } \
//    } profile_scope_##__LINE__(section_name)
//#else
...
//#endif
//
//// Internal structure for building the skeleton
//struct FBXJoint {
//    std::string name;
...
//};
//
//// Check if a node has translation animation (used to detect static reference nodes)
//static bool FBXNodeHasTranslationAnimation(const ufbx_scene* scene, const ufbx_node* node)
//{
//    //FBX_PROFILE_SCOPE("FBXNodeHasTranslationAnimation");
//
...
//                    // Check if this is the "Lcl Translation" property
...
//static Matrix FBXToRaylibMatrix(const ufbx_matrix* m)
//{
//    return Matrix{
...
//// BVH uses ZXY order: R = Rz * Rx * Ry
//static Vector3 FBXMatrixToEulerZXY(const Matrix& mat)
//{
//    //FBX_PROFILE_SCOPE("FBXMatrixToEulerZXY");
//
//    // Normalize columns to handle any scaling in the matrix
...
//
//    // Extract relevant matrix elements (row, col)
...
//}
//
//// Recursively collect nodes into joints vector
...
//    float offsetScale)
//{
//    //FBX_PROFILE_SCOPE("FBXCollectNodes");
//
//    FBXJoint j;
...
//    // Calculate offset
//    if (parentIdx == -1) {
//        // Root: use world position (already in meters from ufbx)
...
//    }
//    else {
//        // Child: use local offset (in original FBX units, typically cm)
...
//}
//
//// Collect joint indices in hierarchy order (for motion data output)
...
//}
//
//// Main FBX loading function - populates BVHData structure
//static bool FBXDataLoadOld(BVHData* bvh, const char* filename, char* errMsg, int errMsgSize)
//{
//#ifdef FBX_PROFILING
//    double total_start = GetTime();
//    printf("\n=== Old FBX Loader Profiling ===\n");
...
//    const float offsetScale = 0.01f;  // cm -> meters for offsets
//
//    // Build skeleton
...
//    }
//
//    // Add end sites to leaf joints
//    FBX_PROFILE_START(endsites_time);
//
//    const size_t jointCountBeforeEndSites = joints.size();
...
//            end.offset = Vector3{ 0.0f, 0.05f, 0.0f };  // Default end site offset
//            end.isEndSite = true;
...
//
//    // Free and re-init BVH data
//    BVHDataFree(bvh);
//    BVHDataInit(bvh);
//
//    // Allocate joints (including end sites that are children of hierarchy joints)
//    // We need to count total joints in the hierarchy tree (including end sites)
...
//        for (int child : joints[idx].children) {
//            collectAll(child);
//        }
//        };
//    collectAll(rootIdx);
...
//    bvh->joints = (BVHJointData*)malloc(bvh->jointCount * sizeof(BVHJointData));
//
//    // Map from FBXJoint index to BVHJointData index
//    std::vector<int> fbxToBvhIdx(joints.size(), -1);
//    for (int i = 0; i < (int)fullHierarchyOrder.size(); ++i) {
//        fbxToBvhIdx[fullHierarchyOrder[i]] = i;
//    }
//
//    FBX_PROFILE_END(joint_allocation_time, "Joint Allocation & Index Mapping");
//
//    // Populate joint data
...
//            // Root: 6 channels
//            bj->channelCount = 6;
//            bj->channels[0] = CHANNEL_X_POSITION;
//            bj->channels[1] = CHANNEL_Y_POSITION;
//            bj->channels[2] = CHANNEL_Z_POSITION;
//            bj->channels[3] = CHANNEL_Z_ROTATION;
//            bj->channels[4] = CHANNEL_X_ROTATION;
//            bj->channels[5] = CHANNEL_Y_ROTATION;
//        }
//        else {
//            // Non-root: 3 channels
//            bj->channelCount = 3;
//            bj->channels[0] = CHANNEL_Z_ROTATION;
//            bj->channels[1] = CHANNEL_X_ROTATION;
//            bj->channels[2] = CHANNEL_Y_ROTATION;
//        }
//    }
//
...
//    bvh->channelCount = 0;
//    for (int i = 0; i < bvh->jointCount; ++i) {
//        bvh->channelCount += bvh->joints[i].channelCount;
//    }
//
//    // Extract animation
...
//    bvh->motionData = (float*)malloc(bvh->frameCount * bvh->channelCount * sizeof(float));
//
//    // Extract frame data
...
//        ufbx_scene* evalScene = ufbx_evaluate_scene(scene, stack->anim, time, nullptr, &evalError);
...
//
//            const ufbx_node* evalNode = evalScene->nodes.data[fj.node->typed_id];
...
//                bvh->motionData[f * bvh->channelCount + channelOffset + 0] = (float)evalNode->node_to_world.m03;
//                bvh->motionData[f * bvh->channelCount + channelOffset + 1] = (float)evalNode->node_to_world.m13;
//                bvh->motionData[f * bvh->channelCount + channelOffset + 2] = (float)evalNode->node_to_world.m23;
...
//    printf("Old  Total FBX loading time: %.3f ms\n", total_elapsed * 1000.0);
...
//    // --- OPTIMIZATION START ---
//
//    // We reuse the same storage for the loop to avoid allocations
//    // ufbx_evaluate_transform is lightweight and doesn't allocate memory.
//
...
//            // Instead of evaluating the whole scene, we only evaluate the specific 
//            // transform for this node at this time.
//            const ufbx_transform xform = ufbx_evaluate_transform(stack->anim, fj.node, time);
//
//            // Convert quaternion to rotation matrix
//            Quaternion q = {
...
//            };
//
//            // Optimization: Only compute Matrix/Euler if we actually need rotation channels
//            // (Most BVH joints do, but good to be explicit)
//            Matrix rotMat = QuaternionToMatrix(q);
//            Vector3 euler = FBXMatrixToEulerZXY(rotMat);
//
//            if (fj.parentIndex == -1) {
//                // Root: Position + Rotation
//                // Note: We use xform.translation (Local) instead of node_to_world. 
//                // For the root of a skeleton, Local is usually equivalent to World 
//                // unless the skeleton is parented to a moving object not captured here.
//                bvh->motionData[f * bvh->channelCount + channelOffset + 0] = (float)xform.translation.x;
//                bvh->motionData[f * bvh->channelCount + channelOffset + 1] = (float)xform.translation.y;
//                bvh->motionData[f * bvh->channelCount + channelOffset + 2] = (float)xform.translation.z;
//                bvh->motionData[f * bvh->channelCount + channelOffset + 3] = euler.z;
//                bvh->motionData[f * bvh->channelCount + channelOffset + 4] = euler.x;
//                bvh->motionData[f * bvh->channelCount + channelOffset + 5] = euler.y;
//                channelOffset += 6;
